...
$(document).ready(init)
$(document).on('mousedown', '.table-header', startDrag)
...
function init() {
	...
	renderTables(tables)
	layoutTables()
	$('#diagram').data('relationships', relationships)
	redrawLines()
}
...
function layoutTables() {
	...
	grid.find('.table-box').each(each_box)
	...
	function each_box() {
		...
		positions.push({ el: self, top: pos.top, left: pos.left, width: w, height: h })
		if (pos.top + h > maxBottom) maxBottom = pos.top + h
	}

	grid.css({ display: 'block', minHeight: maxBottom + 24 })

	positions.forEach(each_position)

	function each_position(p) {
		p.el.css({ position: 'absolute', left: p.left, top: p.top, width: p.width })
	}
}
...
function redrawLines() {
	var relationships = $('#diagram').data('relationships')
	var diagram       = document.getElementById('diagram')
...
function startDrag(e) {
	var box = $(this).closest('.table-box')
	dragState = {
		box:    box,
...
function doDrag(e) {
	if (!dragState) return
	dragState.box.css({
		left: e.pageX - dragState.startX,
		top:  e.pageY - dragState.startY
	})
	redrawLines()
}
